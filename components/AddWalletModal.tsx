  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="p-6">
          <label htmlFor="wallet-name" className="block text-slate-500 dark:text-slate-400 mb-2">Wallet Name</label>
          <input
            id="wallet-name"
            type="text"
        ...
        <div className="p-6 bg-slate-50 dark:bg-slate-800/50 border-t border-slate-200 dark:border-slate-700 rounded-b-lg flex justify-end space-x-4">
          ...
          <button onClick={handleSave} disabled={!name.trim()} className="bg-cyan-500 hover:bg-cyan-600 text-white font-bold py-2 px-4 rounded-lg transition-colors disabled:bg-slate-300 dark:disabled:bg-slate-700 disabled:text-slate-500 dark:disabled:text-slate-500 disabled:cursor-not-allowed">
            Create Wallet
          </button>
        </div>
      </div>
