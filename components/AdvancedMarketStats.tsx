...
const LoadingSkeletonCard = () => (
    <div className="bg-white dark:bg-slate-800 p-6 rounded-lg animate-pulse">
    ...
    return (
        <div className="grid grid-cols-1 gap-4">
            <StatCard title={open_interest.name}>
                <p className="text-2xl font-bold text-slate-900 dark:text-white">{open_interest.value}</p>
                <p className={`text-md font-mono mt-1 ${oiChange >= 0 ? 'text-green-500' : 'text-red-500'}`}>{oiChange.toFixed(2)}%</p>
            </StatCard>
            <StatCard title={liquidations.name}>
                <p className="text-2xl font-bold text-slate-900 dark:text-white">{liquidations.value}</p>
                <p className={`text-md font-mono mt-1 ${liqChange >= 0 ? 'text-green-500' : 'text-red-500'}`}>{liqChange.toFixed(2)}%</p>
            </StatCard>
            <StatCard title={avg_rsi.name}>
                 <p className="text-2xl font-bold text-slate-900 dark:text-white">{avg_rsi.value}</p>
                 <p className="text-md text-slate-500 dark:text-slate-400 mt-1">{avg_rsi.sentiment}</p>
            </StatCard>
            <StatCard title={altcoin_season_index.name}>
                <p className="text-2xl font-bold text-slate-900 dark:text-white">{altcoin_season_index.value}</p>
                <p className="text-md text-slate-500 dark:text-slate-400 mt-1">{altcoin_season_index.sentiment}</p>
            </StatCard>
        </div>
    );
};
