...
const AddTransactionModal: React.FC<AddTransactionModalProps> = ({ asset, currentQuantity, onClose, onAddTransaction }) => {
  ...
  const commonButtonClasses = 'relative inline-flex items-center justify-center w-1/2 px-4 py-2 border border-slate-600 text-sm font-medium focus:z-10 focus:outline-none focus:ring-2 focus:ring-cyan-500 transition-colors';
            ...
            <div>
                ...
                <div className="grid grid-cols-2 rounded-md shadow-sm">
                    <button
                        type="button"
                        onClick={() => setType('buy')}
                        className={`${commonButtonClasses} rounded-tl-md ${type === 'buy' ? 'bg-cyan-500 text-white' : inactiveButtonClasses}`}
                    >
                        Buy
                    </button>
                    <button
                        ...
                        className={`${commonButtonClasses} rounded-tr-md -ml-px ${type === 'sell' ? 'bg-pink-500 text-white' : inactiveButtonClasses}`}
                    >
                        Sell
                    </button>
                     <button
                        ...
                        className={`${commonButtonClasses} rounded-bl-md -mt-px ${type === 'transfer_in' ? 'bg-emerald-500 text-white' : inactiveButtonClasses}`}
                    >
                        Transfer In
                    </button>
                    <button
                        ...
                        className={`${commonButtonClasses} rounded-br-md -ml-px -mt-px ${type === 'transfer_out' ? 'bg-amber-500 text-white' : inactiveButtonClasses}`}
                    >
                        Transfer Out
                    </button>
                </div>
