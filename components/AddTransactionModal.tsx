...
const AddTransactionModal: React.FC<AddTransactionModalProps> = ({ asset, currentQuantity, currentPrice, onClose, onAddTransaction }) => {
  ...
  const commonButtonClasses = 'relative inline-flex items-center justify-center w-full px-4 py-2 border text-sm font-medium focus:z-10 focus:outline-none focus:ring-2 focus:ring-offset-2 dark:focus:ring-offset-slate-800 focus:ring-cyan-500 transition-colors';
  const inactiveButtonClasses = 'bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-600 border-slate-300 dark:border-slate-600';
    ...
    >
      ...
      >
        ...
        <div className="p-6 space-y-4">
            <div>
                <label className={`${labelClasses} mb-2`}>Transaction Type</label>
            ...
            <div>
                <div className="flex justify-between items-center mb-1">
                    <label htmlFor="tx-quantity" className={labelClasses}>Quantity</label>
                    {isDebit && (
                        <button 
                            type="button"
             ...
             {!isTransfer && (
                <div>
                    <div className="flex justify-between items-center mb-1">
                        <label htmlFor="tx-price" className={labelClasses}>Price Per Coin (USD)</label>
                        {currentPrice > 0 && (
                            <button
                                type="button"
            ...
            <div>
              <label htmlFor="tx-fee" className={`${labelClasses} mb-1`}>Fee (USD) (Optional)</label>
              <input
                  id="tx-fee"
                  type="number"
            ...
            <div>
                <label htmlFor="tx-date" className={`${labelClasses} mb-1`}>Date</label>
                <input
                    id="tx-date"
                    type="date"
                    value={date}
             ...
             <div>
                <label htmlFor="notes" className={`${labelClasses} mb-1`}>Notes (Optional)</label>
                <textarea
                  id="notes"
