...
const TransactionHistory: React.FC<{ transactions: Transaction[]; isPrivacyMode: boolean }> = ({ transactions, isPrivacyMode }) => {
    ...
    const typeClasses: Record<string, string> = {
        buy: 'bg-green-500/10 text-green-500 dark:text-green-400',
        sell: 'bg-red-500/10 text-red-500 dark:text-red-400',
        transfer_in: 'bg-blue-500/10 text-blue-500 dark:text-blue-400',
        transfer_out: 'bg-yellow-500/10 text-yellow-500 dark:text-yellow-400',
    };
    ...
    return (
        <div className="px-4 py-3 bg-slate-100 dark:bg-slate-900/50">
            <h4 className="text-sm font-semibold text-slate-900 dark:text-white mb-2 px-3">Transaction History</h4>
            <div className="max-h-60 overflow-y-auto">
                <table className="w-full text-left text-xs">
                    <thead className="sticky top-0 bg-slate-100/80 dark:bg-slate-900/50 backdrop-blur-sm z-10">
                    ...
                    <tbody>
                        {sortedTransactions.map(tx => (
                             <tr key={tx.id} className="border-b border-slate-200 dark:border-slate-700/50">
                                <td className="p-3">
                                    <span className={`px-2 py-1 text-xs font-semibold rounded-full ${typeClasses[tx.type]}`}>
                                        {formatTxType(tx.type)}
                                    </span>
                                </td>
                                <td className="p-3 font-mono text-slate-700 dark:text-slate-300">{new Date(tx.date).toLocaleDateString('en-CA')}</td>
                                <td className="p-3 font-mono text-slate-800 dark:text-slate-200 text-right">{tx.quantity.toLocaleString(undefined, { maximumFractionDigits: 8 })}</td>
                                <td className="p-3 font-mono text-slate-700 dark:text-slate-300 text-right">
                                    {tx.type === 'buy' || tx.type === 'sell' ? (isPrivacyMode ? '$ ****' : `$${tx.pricePerUnit.toLocaleString(undefined, {style: 'currency', currency: 'USD'}).replace('$', '')}`) : '-'}
                                </td>
                                <td className="p-3 font-mono text-slate-700 dark:text-slate-300 text-right">
                                    {tx.fee ? (isPrivacyMode ? '$ ****' : formatCurrency(tx.fee)) : <span className="text-slate-400 dark:text-slate-500">-</span>}
                                </td>
                                <td className="p-3 text-slate-600 dark:text-slate-400 max-w-[150px] whitespace-normal break-words">{tx.notes || <span className="text-slate-400 dark:text-slate-500">-</span>}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
                ...
                <tr className="border-b border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400">
                    <th className="py-3 px-2 font-medium text-center">
                        <button
                            ...
                            className="flex items-center justify-center w-full group text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors"
                            title="Sort by Rank"
                        >
                            #
                            <span className="ml-1">
                    ...
                    <th className="py-3 px-4 font-medium text-right">
                        <button
                            ...
                            className="flex items-center justify-end w-full group text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors"
                            title="Sort by 24h %"
                        >
                            24h %
                            <span className="ml-1">
                    ...
                    <th className="py-3 px-4 font-medium text-right">
                        <button
                            onClick={() => onSortChange('change7d')}
                            className="flex items-center justify-end w-full group text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors"
                            title="Sort by 7d %"
                        >
                            7d %
                            <span className="ml-1">
                                {sortConfig.key === 'change7d' && sortConfig.direction === 'asc' ? <ArrowUpIcon className="h-4 w-4" /> :
                                 sortConfig.key === 'change7d' && sortConfig.direction === 'desc' ? <ArrowDownIcon className="h-4 w-4" /> :
                                 <div className="h-4 w-4" />
                                }
                            </span>
                        </button>
                ...
                {assets.map(asset => {
                    ...
                    return (
                        <React.Fragment key={asset.id}>
                            <tr className="border-b border-slate-200 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-700/50">
                                <td 
                                    className="py-4 px-2 text-center font-mono text-slate-500 dark:text-slate-400 cursor-pointer"
                                ...
                                <td className="py-4 px-4">
                                    ...
                                    >
                                        <div className="flex flex-col">
                                            <span className="font-bold text-base text-slate-900 dark:text-white group-hover:text-cyan-400 transition-colors">{asset.name}</span>
                                            <span className="text-xs text-slate-500 dark:text-slate-400 uppercase">{asset.symbol}</span>
                                        </div>
                                    </a>
                                </td>
                                <td className="py-4 px-4 text-right font-mono text-slate-800 dark:text-white">{formatNumber(currentQuantity)}</td>
                                <td className="py-4 px-4 text-right font-mono text-slate-600 dark:text-slate-300">{isPrivacyMode ? '$ ****' : formatCurrency(avgBuyPrice)}</td>
                                <td className="py-4 px-4 text-right font-mono text-slate-800 dark:text-slate-100">
                                    {isPriceLoading ? <div className="h-5 bg-slate-200 dark:bg-slate-700 rounded animate-pulse w-20 ml-auto"></div> : (isPrivacyMode ? '$ ****' : formatCurrency(currentPrice))}
                                </td>
                                <td className="py-4 px-4 text-right font-mono">
                                    {isPriceLoading ? (
                                        <div className="h-5 bg-slate-200 dark:bg-slate-700 rounded animate-pulse w-16 ml-auto"></div>
                                    ) : (
                                ...
                                <td className="py-4 px-4 text-right font-mono">
                                    {isPriceLoading ? <div className="h-5 bg-slate-200 dark:bg-slate-700 rounded animate-pulse w-20 ml-auto"></div> : <ProfitLoss value={unrealizedPL} isPrivacyMode={isPrivacyMode} />}
                                </td>
                                <td className="py-4 px-4 text-right font-mono text-slate-800 dark:text-white font-bold">
                                    {isPriceLoading ? <div className="h-5 bg-slate-200 dark:bg-slate-700 rounded animate-pulse w-24 ml-auto"></div> : (isPrivacyMode ? '$ ****' : formatCurrency(marketValue))}
                                </td>
                                <td className="py-4 px-4 text-center">
                                    <div className="flex items-center justify-center space-x-1">
                                        <button 
                                            onClick={(e) => { e.stopPropagation(); onAddTransaction(asset); }}
                                            className="text-slate-500 dark:text-slate-400 hover:text-cyan-400 transition-colors p-2 rounded-full"
                                        ...
                                        <button 
                                            ...
                                            className="text-slate-500 dark:text-slate-400 hover:text-red-500 transition-colors p-2 rounded-full"
