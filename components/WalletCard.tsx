        ...
        <table className="w-full text-left text-sm">
            ...
            <tbody>
                {assets.map(asset => {
                    ...
                    return (
                        <tr key={asset.id} className="border-b border-slate-200 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-700/50">
                             <td className="py-4 px-2 text-center font-mono text-slate-500 dark:text-slate-400">
                                {priceInfo?.market_cap_rank ?? '-'}
                            </td>
                            <td className="py-4 px-4">
                                <div className="flex flex-col">
                                    <span className="font-bold text-base text-slate-900 dark:text-white">{asset.name}</span>
                                    <span className="text-xs text-slate-500 dark:text-slate-400 uppercase">{asset.symbol}</span>
                                </div>
                            </td>
                            <td className="py-4 px-4 text-right font-mono text-slate-900 dark:text-white">{formatNumber(currentQuantity)}</td>
                            <td className="py-4 px-4 text-right font-mono text-slate-600 dark:text-slate-300">{formatCurrency(avgBuyPrice)}</td>
                            <td className="py-4 px-4 text-right font-mono text-slate-800 dark:text-slate-100">
                                {isPriceLoading ? <div className="h-5 bg-slate-200 dark:bg-slate-700 rounded animate-pulse w-20 ml-auto"></div> : formatCurrency(currentPrice)}
                            </td>
                            ...
                            <td className="py-4 px-4 text-right font-mono">
                                {isPriceLoading ? <div className="h-5 bg-slate-200 dark:bg-slate-700 rounded animate-pulse w-20 ml-auto"></div> : <ProfitLoss value={unrealizedPL} />}
                            </td>
                            <td className="py-4 px-4 text-right font-mono text-slate-900 dark:text-white font-bold">
                                 {isPriceLoading ? <div className="h-5 bg-slate-200 dark:bg-slate-700 rounded animate-pulse w-24 ml-auto"></div> : formatCurrency(marketValue)}
                            </td>
                            <td className="py-4 px-4 text-center">
                                <div className="flex items-center justify-center space-x-1">
                                    <button 
                                        onClick={() => onAddTransaction(asset)} 
                                        className="text-slate-500 dark:text-slate-400 hover:text-cyan-500 dark:hover:text-cyan-400 transition-colors p-2 rounded-full"
