...
import { streakContractAddress, streakContractAbi } from '../services/ethereumStreakContract';
...
const ETHEREUM_CHAIN_ID = '0x1'; // 1 in hex
const ETHEREUM_CHAIN_ID_DECIMAL = 1;
const ETHEREUM_RPC_URL = 'https://cloudflare-eth.com';

export function useEthereumStreak() {
    ...
    const switchNetwork = useCallback(async (ethProvider: Eip1193Provider) => {
        try {
            ...
            // Mainnet is almost always present, so we don't try to add it.
            // If it fails, it's likely user rejection.
    ...
    const connectWallet = useCallback(async () => {
        ...
        try {
            ...
            if (network.chainId !== BigInt(ETHEREUM_CHAIN_ID_DECIMAL)) {
    ...
    useEffect(() => {
        const handleAccountsChanged = (accounts: string[]) => {
            if (accounts.length === 0) {
                ...
                // Reconnect to reflect the new account and fetch its streak
                connectWallet();
            }
        };
        
        const handleChainChanged = () => {
            // Reload to ensure the provider is correctly configured for the new chain
