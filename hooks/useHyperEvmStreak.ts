...
import { streakContractAddress, streakContractAbi } from '../services/hyperEvmStreakContract';
...
export function useHyperEvmStreak() {
            ...
            if (switchError.code === 4902) { // Chain not added
                try {
                    await ethProvider.request({
                        ...
                        params: [HYPEREVM_CHAIN_PARAMS],
                    });
                } catch (addError) {
                    throw new Error("Failed to add HyperEVM network.");
                }
            } else {
                throw new Error("Failed to switch to HyperEVM network.");
            }
        }
    }, []);
    ...
    const connectWallet = useCallback(async () => {
        ...
        try {
            ...
            if (network.chainId !== BigInt(HYPEREVM_CHAIN_ID_DECIMAL)) {
